const splitResult = (state, dispatch) => {
  ...
  let balance = resultingBalance - state.playerBalance;
  // Dispatch outcome based on overall result
  if (playerWins > dealerWins) {
    dispatch({
      type: "OUTCOME",
      payload: "Player Wins! Resulting Balance: $" + balance,
    });
    ...
    if (resultingBalance <= 0) {
      ...
      dispatch({
        type: "OUTCOME",
        payload: "Dealer Wins! Resulting Balance: $" + balance,
      });
    }
  } else {
    dispatch({
      type: "OUTCOME",
      payload: "Tie! Resulting Balance: $" + balance,
    });
  }
